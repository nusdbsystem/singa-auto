...
class MyModel(sonnx.SONNXModel):
    ...
    def forward(self, *x):
        ...
        y = super(MyModel, self).forward(*x, last_layers=-4)[0]
        y = self.conv(y)
        y = tensor.reshape(y, y.shape[:2])
        return y
...
class OnnxMobileNet(OnnxModelBase):
    '''
    Implements MobileNet on the ONNX of SINGA for IMAGE_CLASSIFICATION
    '''
