...
f.DEFINE_integer('feature_win_len', 32,
                 'feature extraction audio window length in milliseconds')
f.DEFINE_integer('feature_win_step', 20,
                 'feature extraction window step length in milliseconds')
f.DEFINE_integer('audio_sample_rate', 16000,
                 'sample rate value expected by model')

# Global Constants
# ================

f.DEFINE_float('dropout_rate', 0.05, 'dropout rate for feedforward layers')
f.DEFINE_float('dropout_rate2', -1.0,
               'dropout rate for layer 2 - defaults to dropout_rate')
f.DEFINE_float('dropout_rate3', -1.0,
               'dropout rate for layer 3 - defaults to dropout_rate')
f.DEFINE_float('dropout_rate4', 0.0,
               'dropout rate for layer 4 - defaults to 0.0')
f.DEFINE_float('dropout_rate5', 0.0,
               'dropout rate for layer 5 - defaults to 0.0')
f.DEFINE_float('dropout_rate6', -1.0,
               'dropout rate for layer 6 - defaults to dropout_rate')
...
f.DEFINE_integer('checkpoint_secs', 600,
                 'checkpoint saving interval in seconds')
f.DEFINE_integer('max_to_keep', 3,
                 'number of checkpoint files to keep - default value is 5')
...
f.DEFINE_integer('random_seed', 4568,
                 'default random seed that is used to initialize variables')

# Early Stopping (when PolicyKnob('EARLY_STOP') is used)

f.DEFINE_float(
    'es_dev_ratio', 0.05,
    'Proportion of the training set to be used for early stopping validation')
f.DEFINE_integer('es_steps', 4,
                 'number of validations to consider for early stopping')
f.DEFINE_float(
    'es_mean_th', 0.5,
...
f.DEFINE_string(
    'lm_trie_path', 'tfdeepspeech/trie',
...
f.DEFINE_string('query_path', '',
                'Path(s) to query audio(s), delimited by commas')
...
class TfDeepSpeech(BaseModel):
    ...
    def initialize_globals(self):
        ...
        c.audio_window_samples = FLAGS.audio_sample_rate * (
    ...
    def __init__(self, **knobs):
        ...
        self._knobs = knobs
        self._sess_config = tf.ConfigProto(allow_soft_placement=True,
    ...
    def audiofile_to_features(self, wav_bytes):
        ...
        features, features_len = self.samples_to_mfccs(decoded.audio,
                                                       decoded.sample_rate)
    ...
    def create_dataset(self,
        ...
        def to_sparse_tuple(sequence):
            ...
            indices = np.asarray(list(
                zip([0] * len(sequence), range(len(sequence)))),
                                 dtype=np.int64)
        ...
        def generate_values():
            for _, row in df.iterrows():
                yield os.path.join(dataset_dir.name,
                                   row.wav_filename), to_sparse_tuple(
                                       row.transcript)
        ...
        dataset = utils.dataset.load_dataset_of_audio_files(
        ...
        if batch_size > dataset_size:
            logger.log(
                f'Capping batch size to {dataset_size} (length of dataset)...')
        ...
        df['transcript'] = df['transcript'].apply(
            partial(text_to_char_array, alphabet=Config.alphabet))
        ...
        dataset = (tf.data.Dataset.from_generator(
            ...
            output_types=(tf.string, (
                tf.int64, tf.int32,
                tf.int64))).map(entry_to_features).cache(cache_path).window(
                    batch_size,
                    drop_remainder=True).flat_map(batch_fn).prefetch(num_gpus))
    ...
    def rnn_impl_lstmblockfusedcell(self, x, seq_length, previous_state, reuse):
        ...
        fw_cell = tf.contrib.rnn.LSTMBlockFusedCell(self.c.n_cell_dim,
                                                    reuse=reuse)
    ...
    def create_overlapping_windows(self, batch_x):
        ...
        batch_x = tf.reshape(batch_x,
                             [batch_size, -1, window_width, num_channels])
    ...
    def create_model(self,
        ...
        def dense(name, x, units, dropout_rate=None, relu=True):
            with tf.variable_scope(name):
                bias = self.variable_on_cpu('bias', [units],
                                            tf.zeros_initializer())
                weights = self.variable_on_cpu(
                    'weights', [x.shape[-1], units],
        ...
        batch_x = tf.reshape(
            batch_x,
            [-1, Config.n_input + 2 * Config.n_input * Config.n_context
            ])  # (n_steps*batch_size, n_input + 2*n_input*n_context)
        layers['input_reshaped'] = batch_x

        # The next three blocks will pass `batch_x` through three hidden layers with
        # clipped RELU activation and dropout.
        layers['layer_1'] = layer_1 = dense('layer_1',
                                            batch_x,
                                            Config.n_hidden_1,
                                            dropout_rate=dropout[0])
        layers['layer_2'] = layer_2 = dense('layer_2',
                                            layer_1,
                                            Config.n_hidden_2,
                                            dropout_rate=dropout[1])
        layers['layer_3'] = layer_3 = dense('layer_3',
                                            layer_2,
                                            Config.n_hidden_3,
        ...
        layers['layer_5'] = layer_5 = dense('layer_5',
                                            output,
                                            Config.n_hidden_5,
                                            dropout_rate=dropout[5])

        # Now we apply a final linear layer creating `n_classes` dimensional vectors, the logits.
        layers['layer_6'] = layer_6 = dense('layer_6',
                                            layer_5,
                                            Config.n_hidden_6,
                                            relu=False)
        ...
        layer_6 = tf.reshape(layer_6, [-1, batch_size, Config.n_hidden_6],
                             name='raw_logits')
    ...
    def calculate_mean_edit_distance_and_loss(self, iterator, dropout, reuse):
        ...
        logits, _ = self.create_model(batch_x,
                                      batch_seq_len,
                                      dropout,
                                      reuse=reuse)
    ...
    def get_tower_results(self, iterator, optimizer, dropout_rates):
            ...
            for i in range(len(Config.available_devices)):
                ...
                with tf.device(device):
                    ...
                    with tf.name_scope('tower_%d' % i):
                        ...
                        avg_loss = self.calculate_mean_edit_distance_and_loss(
        ...
        tf.summary.scalar(name='step_loss',
                          tensor=avg_loss_across_towers,
    ...
    def log_variable(self, variable, gradient=None):
        ...
        tf.summary.scalar(name='%s/sttdev' % name,
                          tensor=tf.sqrt(
                              tf.reduce_mean(tf.square(variable - mean))))
        ...
        if gradient is not None:
            ...
            if grad_values is not None:
                tf.summary.histogram(name='%s/gradients' % name,
                                     values=grad_values)
    ...
    def try_loading(self, session, saver, checkpoint_filename, caption):
        try:
            ...
            logger.log('Restored variables from %s checkpoint at %s, step %d' %
    ...
    def create_inference_graph(self, batch_size=1, n_steps=16):
        ...
        input_tensor = tf.placeholder(tf.float32, [
            batch_size, n_steps if n_steps > 0 else None,
            2 * Config.n_context + 1, Config.n_input
        ],
                                      name='input_node')
        ...
        if not batch_size or batch_size <= 0:
            ...
            previous_state_c = self.variable_on_cpu(
                'previous_state_c', [batch_size, Config.n_cell_dim],
                initializer=None)
            previous_state_h = self.variable_on_cpu(
                'previous_state_h', [batch_size, Config.n_cell_dim],
                initializer=None)
            previous_state = tf.contrib.rnn.LSTMStateTuple(
        ...
        logits, layers = self.create_model(
        ...
        initialize_state = tf.group(initialize_c,
                                    initialize_h,
    ...
    def train(self, dataset_path, **kwargs):
        ...
        logger.log('Train dataset will be extracted to {}'.format(
            dataset_dir.name))

        train_set, dev_set = self.create_dataset(dataset_path,
                                                 dataset_dir,
                                                 dataset_type='train')
        ...
        gradients, loss = self.get_tower_results(iterator, optimizer,
                                                 dropout_rates)
        ...
        try:
            for epoch in range(ep):
                ...
                if train_loss == float('inf'):
                    logger.log(
                        'No valid CTC path found due to shorter input sequence than label. '
                        'Bad learning_rate and n_hidden combination. '
                        '(Try other learning_rate and n_hidden combinations)')
                    raise Exception(
                        'Bad learning_rate and n_hidden combination')
                logger.log('Finished training epoch %d - loss: %f' %
                           (epoch, train_loss))
                ...
                if dev_set:
                    ...
                    logger.log('Finished validating epoch %d - loss: %f' %
                               (epoch, set_loss))
                    dev_losses.append(dev_loss)

                    # Early stopping
                    if self._knobs.get(
                            'early_stop') and len(dev_losses) >= FLAGS.es_steps:
    ...
    def evaluate(self, dataset_uri):
        ...
        dataset_dir = tempfile.TemporaryDirectory()
        logger.log('Test dataset will be extracted to {}'.format(
            dataset_dir.name))

        test_set, _ = self.create_dataset(dataset_uri,
                                          dataset_dir,
                                          dataset_type='test')
        ...
        def calculate_report(labels, decodings, losses):
            ...
            def wer_cer_batch(samples):
                ...
                """
                wer = sum(s.word_distance for s in samples) / sum(
                    s.word_length for s in samples)
                cer = sum(s.char_distance for s in samples) / sum(
                    s.char_length for s in samples)
            ...
            def process_decode_result(item):
                ...
                word_distance = levenshtein(ground_truth.split(),
                                            prediction.split())
            ...
            samples = pmap(process_decode_result, zip(labels, decodings,
                                                      losses))
        ...
        def run_test(init_op, dataset):
            ...
            scorer = Scorer(self._knobs.get('lm_alpha'),
                            self._knobs.get('lm_beta'), FLAGS.lm_binary_path,
            ...
            logger.log('Test on %s - WER: %f, CER: %f, loss: %f' %
                       (dataset, wer, cer, mean_loss))
            logger.log('Reporting %i samples with highest WER ...' %
                       FLAGS.report_count)
            logger.log('-' * 80)
    ...
    def predict(self, queries, n_steps=16):
        ...
        new_input_tensor = tf.placeholder(
            tf.float32, [None, None, 2 * Config.n_context + 1, Config.n_input],
            name='input_node')
        new_seq_length = tf.placeholder(tf.int32, [None], name='input_lengths')

        tf.import_graph_def(self._loaded_graph_def, {
            'input_node:0': new_input_tensor,
            'input_lengths:0': new_seq_length
        },
                            name='')
        ...
        for query in queries:
            ...
            # Evaluate
            features = self.create_overlapping_windows(features).eval(
                session=session)
    ...
    def dump_parameters(self):
        ...
        Config = self.c
        tf.reset_default_graph()
        input, outputs, _ = self.create_inference_graph(batch_size=-1,
                                                        n_steps=-1)
        output_names_tensor = [
            tensor.op.name
        ...
        try:
            ...
            params['pb_model_base64'] = base64.b64encode(pb_model_bytes).decode(
                'utf-8')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path',
                        type=str,
                        default='data/ldc93s1/ldc93s1.zip',
    ...
    parser.add_argument('--query_path',
                        type=str,
                        default='data/ldc93s1/ldc93s1/LDC93S1.wav',
