...
def _convert_dataset(dataset_path, out_meta_tsv_path, out_train_dataset_path,
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        sents_filepaths = glob.glob(
            os.path.join(tagged_dirpath, SENTS_FILENAME_GLOB))
        ...
        for sents_filepath in tqdm(sents_filepaths[0:train_files_count],
                                   unit='files'):
        ...
        for sents_filepath in tqdm(sents_filepaths[train_files_count:],
                                   unit='files'):
            with open(sents_filepath) as f:
                while True:
                    sent = _read_next_sentence(f, tag_to_index)
    ...
    os.rename(out_path,
              out_train_dataset_path)  # Remove additional trailing `.zip`
    out_path = shutil.make_archive(out_val_dataset_path, 'zip', val_d.name)
    os.rename(out_path,
              out_val_dataset_path)  # Remove additional trailing `.zip`
