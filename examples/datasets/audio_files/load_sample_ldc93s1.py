...
def load_sample_ldc93s1(data_dir):
    ...
    audio_file = maybe_download(LDC93S1_BASE + ".wav", work_dir,
                                LDC93S1_BASE_URL + LDC93S1_BASE + ".wav")
    trans_file = maybe_download(LDC93S1_BASE + ".txt", work_dir,
    ...
    with open(trans_file, "r") as fin:
        transcript = ' '.join(
            fin.read().strip().lower().split(' ')[2:]).replace('.', '')

    df = pandas.DataFrame(
        data=[(LDC93S1_BASE + ".wav", os.path.getsize(audio_file), transcript)],
    ...
    print('LDC93S1 dataset file is saved at {}'.format(
        os.path.join(data_dir, "ldc93s1.zip")))
