...
(kubectl create -f scripts/kubernetes/start_logstash_service.json \
...
(kubectl create -f scripts/kubernetes/start_logstash_deployment.json \
&> $LOGSTADH_LOG_FILE_PATH) &
ensure_stable "SINGA-Auto's logstash Service" $LOGSTADH_LOG_FILE_PATH 1
...
(kubectl create -f scripts/kubernetes/start_es_service.json \
&> $ES_LOG_FILE_PATH) &
ensure_stable "SINGA-Auto's es Service" $ES_LOG_FILE_PATH 1

ES_LOG_FILE_PATH=$PWD/logs/start_es_deployment.log
(kubectl create -f scripts/kubernetes/start_es_deployment.json \
&> $ES_LOG_FILE_PATH) &
ensure_stable "SINGA-Auto's es Service" $ES_LOG_FILE_PATH 1
...
(kubectl create -f scripts/kubernetes/start_kibana_service.json \
...
(kubectl create -f scripts/kubernetes/start_kibana_deployment.json \
&> $KIBANA_LOG_FILE_PATH) &
ensure_stable "SINGA-Auto's kibana Service" KIBANA_LOG_FILE_PATH 1
