...
class ImageFilesDatasetLazy(ModelDataset):
    ...
    def __getitem__(self, index):
        image = _load_pil_images([self._full_image_paths[index]], mode=self.mode)[0]
        image_class = self._image_classes[index]
        return (image, image_class)
    ...
    def _extract_zip(self, dataset_path):
        # Create temp directory to unzip to
        # Note the temp directory generated by tempfile.mkdtemp() will not be cleaned automatically
        # Thus it should be removed when the ImageFilesDataset object is destoried
        ...
        dataset_zipfile = zipfile.ZipFile(dataset_path, 'r')
        dataset_zipfile.extractall(path=dir_path)
        dataset_zipfile.close()
